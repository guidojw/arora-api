...
exports.verifyTrelloWebhookRequest = (req, _res, next) => {
    const base64Digest = content => {
        return crypto.createHmac('sha1', process.env.TRELLO_TOKEN).update(content).digest('base64')
    }
    const content = JSON.stringify(req.body) + `${req.protocol}://${req.hostname}${req.baseUrl}`
