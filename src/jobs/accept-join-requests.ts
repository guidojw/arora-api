...
export default class AcceptJoinRequestsJob implements BaseJob {
  @inject(TYPES.DiscordMessageJob)
  private readonly discordMessageJob!: DiscordMessageJob

  @inject(TYPES.ExileRepository)
  private readonly exileRepository!: Repository<Exile>

  @inject(TYPES.GroupService)
  private readonly groupService!: GroupService

  @inject(TYPES.HealthCheckJob)
    ...
    for (const joinRequest of joinRequests) {
      ...
      try {
        if (typeof await this.exileRepository.findOne({ where: { groupId, userId } }) !== 'undefined') {
          ...
          await this.discordMessageJob.run(`Declined **${username}**'s join request`)
        } else {
          ...
          await this.discordMessageJob.run(`Accepted **${username}**'s join request`)
        }
